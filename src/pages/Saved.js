...
import { currency_symbols } from "../components/Currencies";
import { CurrencyContext } from "../context/CryptoContext";
function Saved() {
  ...
  useEffect(() => {
    async function fetchData() {
      const save = JSON.parse(localStorage.getItem("coins"));
      const temp = [];
      for (let i = 0; i < save.length; i++) {
        const data = await axios.get(
          `https://api.coingecko.com/api/v3/coins/${
            JSON.parse(localStorage.getItem("coins"))[i]
          }`
        );
        temp.push(data.data);
      }
