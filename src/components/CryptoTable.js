...
function CryptoTable(props) {
  ...
  const [save, setSave] = useState(
    localStorage.getItem("coins") === null ? [] : localStorage.getItem("coins")
  );
  let removeFav = (data) => {
    const old = JSON.parse(localStorage.getItem("coins"));
    const newfav = old.filter((coin) => coin !== data);
    setSave(newfav);
  };
  let handleFav = (e, data) => {
    e.preventDefault();
    if (save.includes(data)) return removeFav(data);
    else {
      const recent = JSON.parse(localStorage.getItem("coins"));
      recent.push(data);
      setSave(recent);
    }
  };
  useEffect(() => {
    ...
    if (
      ...
      save !== [] &&
      JSON.stringify(save) != JSON.stringify(localStorage.getItem("coins"))
    ) {
      localStorage.removeItem("coins");
      localStorage.setItem("coins", JSON.stringify(save));
    }
  }, [save]);
  useEffect(() => {
    async function fetchData() {
      const data = await axios.get(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${currency}&ids=${searchCoin}&order=${orderBy}&per_page=10&page=1&sparkline=false`
      );
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await axios.get(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${currency}&order=${orderBy}&per_page=10&page=${page}&sparkline=false`
      );
      setCoinDetails(data.data);
    }
    fetchData();
  }, [currency, orderBy, page]);
  return (
    <CryptoContext.Provider value={{ page, setPage }}>
      <div className="w-[80%]">
        <table className=" mt-5 table-fixed w-full border-2 border-gray-100 ">
            ...
            {coinDetails &&
              coinDetails.map((coin) => (
                <tr className="hover:bg-gray-100 border-b-4 border-gray-100">
                  <td>
                    <button onClick={(e) => handleFav(e, coin.id)}>
                      <svg
                        className={`${
                          save.includes(coin.id)
                            ? "stroke-cyan"
                            : "stroke-gray-100"
                        } hover:stroke-cyan hover:stroke-[1px]`}
                        width="30"
                        height="30"
                        viewBox="0 0 30 30"
                        fill="cyan"
